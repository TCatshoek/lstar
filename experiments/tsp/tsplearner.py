...
class TSPLearner(Learner):
    def __init__(self, teacher: Teacher, tsp):
    ...
    # Checks if no action is performed more than once
    def _is_valid(self, sequence):
        if len(sequence) < len(self.tsp.problem.cities):
                if len(sequence) > 0:
                    if str(self.tsp.initial_state) in sequence:
                        return False
                    else:
    ...
    @depends_on_S
    def _SA(self):
        return set(filter(self._is_valid, set([tuple(chain.from_iterable(x)) for x in list(product(self.S, self.A))]).union(self.A)))
    ...
    def _SUSA(self) -> Set[Tuple]:
        ...
        return set(filter(self._is_valid, self.S.union(SA)))
    ...
    def _is_consistent(self):
        ...
        for (s1, s2) in eqrows:
            for a in self.A:
                if self._is_valid(s1 + a) and self._is_valid(s2 + a):
