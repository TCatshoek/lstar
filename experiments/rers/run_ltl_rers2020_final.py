...
#fuzzer_basepath = "/home/tom/rersfinal/tomlappy"
fuzzer_basepath = "/home/tom/rersfinal/mamlappy/libfuzz/"

problems = [f'Problem{x}' for x in range(9,10)]
...
for problem in problems:
    ...
    constrpath = f'{rers_basepath}/{problemset}/{problem}/constraints-{problem}.txt'
    mappingpath = f'{rers_basepath}/{problemset}/{problem}/{problem}_alphabet_mapping_C_version.txt'

    sul = RERSSOConnector(path)

    fuzzer_dir = f'{fuzzer_basepath}/{problemset}/{problem}'
    ...
    eqc = StackedChecker(
        ...
        LibFuzzerEquivalenceChecker(sul,
                                    corpus_path=Path(fuzzer_dir).joinpath("corpus"),
                                    fuzzer_path=Path(fuzzer_dir).joinpath(f'{problem}_fuzz'),
                                    minimize=False),
